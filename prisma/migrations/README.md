...
This directory contains Prisma migrations for the OpenMemory database.
...
```bash
DATABASE_URL=postgresql://user@localhost:5432/openmemory npx prisma migrate resolve --applied 0_init
```
...
# 2. Create migration
DATABASE_URL=postgresql://user@localhost:5432/openmemory npx prisma migrate dev --name your_migration_name

# 3. Apply in production
DATABASE_URL=postgresql://user@localhost:5432/openmemory npx prisma migrate deploy
```

### Check Migration Status

```bash
DATABASE_URL=postgresql://user@localhost:5432/openmemory npx prisma migrate status
```

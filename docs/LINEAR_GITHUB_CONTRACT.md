...
This document defines the contract between OpenMemory and Linear's native GitHub integration for PR linking and issue lifecycle management.

## Overview

OpenMemory creates Linear issues with proper structure, but relies on Linear's built-in GitHub integration to handle PR linking and auto-closing. This avoids duplicate work and ensures alignment with Linear's behavior.
...
OpenMemory maintains an internal mapping:
```
...
- [ ] Team ID configured in OpenMemory `.env` (`PM_TOOL_TEAM_ID`)
